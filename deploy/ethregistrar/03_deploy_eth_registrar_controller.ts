...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (resolver === ethers.constants.AddressZero) {
    console.log(
      `No resolver set for .eth; not setting interface ${interfaceId} for TRX Registrar Controller`,
    )
    return
  }
  ...
  console.log(
    `Setting ETHRegistrarController interface ID ${interfaceId} on .trx resolver (tx: ${tx3.hash})...`,
  )
  await tx3.wait()
}
