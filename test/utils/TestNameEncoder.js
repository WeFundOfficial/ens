...
contract('UniversalResolver', function () {
  ...
  describe('encodeName()', () => {
    it('should encode a name', async () => {
      const result = await testNameEncoder.encodeName('foo.trx')
      expect(result['0']).to.equal(dns.hexEncodeName('foo.trx'))
      expect(result['1']).to.equal(namehash('foo.trx'))
    })
    ...
    it('should encode a long name', async () => {
      const result = await testNameEncoder.encodeName('something.else.test.trx')
      expect(result['0']).to.equal(dns.hexEncodeName('something.else.test.trx'))
      expect(result['1']).to.equal(namehash('something.else.test.trx'))
    })
  })
})
